...
const plugin: ProviderPlugin = {
  manifest,
  initialize: async (config) => { apiKey = (config.apiKey as string) ?? process.env.REPLICATE_API_KEY ?? ""; },
  complete: async (request): Promise<LLMCompletionResponse> => {
    ...
    while (result.status !== "succeeded" && result.status !== "failed") {
      ...
      result = (await pollResp.json()) as { id: string; status: string; output?: string[] };
    }
  ...
  healthCheck: async () => { try { await fetch("https://api.replicate.com/v1/models", { headers: { Authorization: `Bearer ${apiKey}` } }); return true; } catch { return false; } },
};
