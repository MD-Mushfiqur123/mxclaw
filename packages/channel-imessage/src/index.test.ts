...
describe("imessage channel", () => {
  ...
  describe("manifest", () => {
    ...
    it("should list sendMessage and receiveMessage capabilities", () => {
      ...
      expect(caps).toContain("tapbacks");
    });
  });
  describe("setupChannel", () => {
    it("should throw when no credentials provided", async () => {
      const config: ChannelConfig = { id: "imessage-1", type: "imessage", enabled: true, credentials: {}, options: {}, allowlist: [], mentionGating: true, pairingEnabled: true };
    ...
    it("should accept credentials from config", async () => {
      ...
      const config: ChannelConfig = { id: "imessage-1", type: "imessage", enabled: true, credentials: { token: "test" }, options: {}, allowlist: [], mentionGating: true, pairingEnabled: true };
