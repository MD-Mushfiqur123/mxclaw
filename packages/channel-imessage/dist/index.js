...
function getState(id) {
    let s = states.get(id);
    if (!s) {
        s = { serverUrl: "", password: "", connected: false, messageCount: 0, queue: [], ws: null };
        states.set(id, s);
    }
    return s;
}
const plugin = {
    ...
    setupChannel: async (config) => {
        const serverUrl = config.credentials?.serverUrl ?? process.env.BLUEBUBBLES_SERVER_URL ?? "http://localhost:1234";
        const password = config.credentials?.password ?? process.env.BLUEBUBBLES_PASSWORD;
        if (!password)
            throw new Error("BlueBubbles password required");
