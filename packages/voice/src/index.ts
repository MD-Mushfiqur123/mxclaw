import type { VoiceConfig, VoicePlugin, PluginManifest } from "@mxclaw/core";
import * as crypto from "node:crypto";

// ── OpenAI Realtime Voice ─────────────────────────────────────────

let openaiApiKey = "";
const openaiSessions = new Map<string, WebSocket>();

export const openaiRealtimeVoice: VoicePlugin = {
  manifest: {
  ...
  initialize: async (config) => {
    openaiApiKey = (config.apiKey as string) ?? process.env.OPENAI_API_KEY ?? "";
    if (!openaiApiKey) throw new Error("OpenAI API key required for realtime voice");
  },
  startSession: async () => {
  ...
  sendAudio: async (sessionId, audio) => {
    ...
    if (!ws || ws.readyState !== WebSocket.OPEN) throw new Error("No active session");
    ws.send(JSON.stringify({
  ...
  receiveAudio: async function* (sessionId) {
    ...
    const handler = (event: MessageEvent) => {
      try {
        const msg = JSON.parse(event.data as string) as { type: string; delta?: string; audio?: string };
...
const elevenLabsSessions = new Map<string, { chunks: Buffer[] }>();

export const elevenlabsVoice: VoicePlugin = {
  manifest: {
    name: "elevenlabs",
    version: "0.2.0",
    type: "voice",
  ...
  initialize: async (config) => {
    elevenLabsKey = (config.apiKey as string) ?? process.env.ELEVENLABS_API_KEY ?? "";
    if (!elevenLabsKey) throw new Error("ElevenLabs API key required");
    elevenLabsVoiceId = (config.voiceId as string) ?? elevenLabsVoiceId;
  },
  startSession: async () => {
    const sessionId = crypto.randomUUID();
...
let geminiApiKey = "";
const geminiSessions = new Map<string, WebSocket>();

export const geminiLiveVoice: VoicePlugin = {
  manifest: {
    name: "gemini-live",
    version: "0.2.0",
    type: "voice",
  ...
  initialize: async (config) => {
    geminiApiKey = (config.apiKey as string) ?? process.env.GEMINI_API_KEY ?? "";
    if (!geminiApiKey) throw new Error("Gemini API key required for live voice");
  },
  startSession: async () => {
    const sessionId = crypto.randomUUID();
  ...
  sendAudio: async (sessionId, audio) => {
    ...
    if (!ws || ws.readyState !== WebSocket.OPEN) throw new Error("No active Gemini session");
    ws.send(JSON.stringify({
      realtimeInput: {
...
export const systemTtsVoice: VoicePlugin = {
  manifest: {
    name: "system-tts",
    version: "0.2.0",
    type: "voice",
...
export class VoiceManager {
  private plugins = new Map<string, VoicePlugin>();
  private activeSessions = new Map<string, string>(); // sessionId -> provider
  ...
  async initialize(config: VoiceConfig): Promise<void> {
    ...
    if (!plugin) throw new Error(`Voice provider "${provider}" not registered`);

    const providerConfig = config[provider as keyof VoiceConfig] ?? {};
    await plugin.initialize(providerConfig as Record<string, unknown>);
  }

  async startVoiceSession(provider: string): Promise<string> {
    const plugin = this.plugins.get(provider);
    if (!plugin) throw new Error(`Voice provider "${provider}" not found`);
  ...
  async sendAudio(sessionId: string, audio: Buffer): Promise<void> {
    ...
    if (!provider) throw new Error(`No active session: ${sessionId}`);

    const plugin = this.plugins.get(provider);
    if (!plugin) throw new Error(`Provider not found: ${provider}`);

    await plugin.sendAudio(sessionId, audio);
  }

  async *receiveAudio(sessionId: string): AsyncGenerator<Buffer> {
    const provider = this.activeSessions.get(sessionId);
    if (!provider) throw new Error(`No active session: ${sessionId}`);
  ...
  async stopSession(sessionId: string): Promise<void> {
    ...
    const plugin = this.plugins.get(provider);
    if (plugin) await plugin.stopSession(sessionId);
...
export async function textToSpeech(
  ...
  switch (provider) {
    case "elevenlabs": {
      const apiKey = (config.apiKey as string) ?? process.env.ELEVENLABS_API_KEY;
      const voiceId = (config.voiceId as string) ?? "21m00Tcm4TlvDq8ikWAM";
    ...
    case "openai": {
      ...
      const response = await fetch("https://api.openai.com/v1/audio/speech", {
        ...
        body: JSON.stringify({
          model: "tts-1",
          input: text,
          voice: (config.voice as string) ?? "alloy",
          response_format: "mp3",
        }),
      });
...
export async function speakText(text: string, config: VoiceConfig): Promise<void> {
  const provider = config.defaultProvider;
  const providerConfig = config[provider as keyof VoiceConfig] ?? {};
  await textToSpeech(text, provider, providerConfig as Record<string, unknown>);
}
