...
function getState(id) {
    let s = states.get(id);
    if (!s) {
        s = { host: "", port: 6667, nick: "mxclaw", channels: [], socket: null, connected: false, messageCount: 0, queue: [], buffer: "" };
        states.set(id, s);
    }
    return s;
}
const plugin = {
    ...
    setupChannel: async (config) => {
        ...
        state.host = config.credentials?.host ?? "irc.libera.chat";
        state.port = config.credentials?.port ?? 6667;
        state.nick = config.credentials?.nick ?? "mxclaw";
        state.channels = config.options?.channels ?? [];
    },
