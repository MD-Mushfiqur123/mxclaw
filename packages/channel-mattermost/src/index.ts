...
interface MMState {
  serverUrl: string; accessToken: string; botUserId: string;
  ws: WebSocket | null; connected: boolean; messageCount: number; queue: ReplyEnvelope[];
  onMessage?: (env: MessageEnvelope) => Promise<void>;
}
const states = new Map<string, MMState>();

function getState(id: string): MMState {
  let s = states.get(id);
  if (!s) { s = { serverUrl: "", accessToken: "", botUserId: "", ws: null, connected: false, messageCount: 0, queue: [] }; states.set(id, s); }
  return s;
}
...
const plugin: ChannelPlugin = {
  ...
  setupChannel: async (config) => {
    const serverUrl = (config.credentials?.serverUrl as string) ?? process.env.MATTERMOST_SERVER_URL;
    const accessToken = (config.credentials?.accessToken as string) ?? process.env.MATTERMOST_ACCESS_TOKEN;
    if (!serverUrl || !accessToken) throw new Error("Mattermost serverUrl and accessToken required");
  ...
  startChannel: async (config, onMessage) => {
    ...
    state.ws.onmessage = (event) => {
      const payload = JSON.parse(event.data as string) as { event: string; data: { post?: string; sender_name?: string; channel_id?: string; text?: string; root_id?: string }; broadcast?: { user_id: string } };
      if (payload.event === "hello") { state.connected = true; return; }
      if (payload.event === "posted" && payload.data.post) {
        const postData = JSON.parse(payload.data.post) as { id: string; user_id: string; channel_id: string; message: string; root_id?: string; create_at: number };
        if (postData.user_id === state.botUserId) return;
