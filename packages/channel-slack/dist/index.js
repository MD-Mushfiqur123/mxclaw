...
const plugin = {
    ...
    setupChannel: async (config) => {
        ...
        const appToken = config.credentials?.appToken ?? process.env.SLACK_APP_TOKEN;
        if (!botToken)
            throw new Error("Slack bot token required (credentials.botToken or SLACK_BOT_TOKEN)");
        if (!appToken)
            throw new Error("Slack app-level token required for Socket Mode (credentials.appToken or SLACK_APP_TOKEN)");
...
async function connectSocketMode(channelId, state) {
...
function handleSlackMessage(channelId, state, evt) {
    const envelope = {
