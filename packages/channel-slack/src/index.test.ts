...
describe("slack channel", () => {
  ...
  describe("setupChannel", () => {
    it("should throw when no credentials provided", async () => {
      const config: ChannelConfig = { id: "slack-1", type: "slack", enabled: true, credentials: {}, options: {}, allowlist: [], mentionGating: true, pairingEnabled: true };
    ...
    it("should accept credentials from config", async () => {
      ...
      const config: ChannelConfig = { id: "slack-1", type: "slack", enabled: true, credentials: { token: "test" }, options: {}, allowlist: [], mentionGating: true, pairingEnabled: true };
