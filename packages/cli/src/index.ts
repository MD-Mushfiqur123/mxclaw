...
import type { MxClawConfig, ChannelConfig, AgentConfig, ProviderRef, BindingConfig } from "@mxclaw/core";
...
program
  ...
  .action(async () => {
    ...
    if (fs.existsSync(configPath)) {
      ...
      try {
        const config = loadConfig();
        const { checkDmPolicySecurity } = await import("@mxclaw/security");
        ...
        if (Object.keys(agents).length === 0) {
          ...
          for (const [id, agent] of Object.entries(agents)) {
            if (!agent.model.provider) {
              ...
              const expectedKeys = providerKeyMap[agent.model.provider];
              if (expectedKeys) {
                // Check if any of the expected keys are present in process.env
                // For a more robust check we could load bootstrap.env, but process.env 
                // contains it if we run via `mxclaw gateway` due to dotenv.
                const hasKey = expectedKeys.some(k => !!process.env[k]);
                if (!hasKey) {
                  // If there's no key in env, it might be in the gateway config or bootstrap
                  // Let's just issue a warning instead of a hard error, since secrets manager
                  // might resolve it at runtime.
                  warnings.push(`Agent "${id}" provider "${agent.model.provider}" typically requires ${expectedKeys[0]}. Ensure it's in your .env or bootstrap.`);
                }
              }
            }
          }
        ...
        if (Object.keys(channels).length === 0) {
          ...
          for (const [id, ch] of Object.entries(channels)) {
            ...
            // DM Policy security check
            const dmWarning = checkDmPolicySecurity(id, ch as any);
            if (dmWarning) {
              warnings.push(`Security: ${dmWarning}`);
            }
          }
        }
    ...
    // Check port conflict (only if config was loaded successfully)
    if (fs.existsSync(configPath)) {
      try {
        const config = loadConfig();
        ...
        const isPortInUse = await new Promise<boolean>((resolve) => {
          ...
          server.listen(port, config.gateway.host);
        });

        if (isPortInUse) {
          issues.push(`Port ${port} is already in use. Gateway will fail to start.`);
        } else {
          ok.push(`Port ${port} is available`);
        }
      } catch {
...
program
  ...
  .action(async (options) => {
    ...
    if (options.edit) {
      const { exec } = await import("node:child_process");
      const editor = process.env.EDITOR ?? process.env.VISUAL ?? (process.platform === "win32" ? "notepad" : "vim");
      if (!fs.existsSync(envPath)) fs.writeFileSync(envPath, "# MxClaw Bootstrap\n", "utf-8");
      exec(`${editor} "${envPath}"`);
      return;
    }
...
program
  .command("pairing")
  .description("Manage active pairing codes from unknown senders")
  .argument("<action>", "Action to perform: list, approve, deny")
  .argument("[code]", "The pairing code (required for approve/deny)")
  .action(async (action, code) => {
    try {
      ...
      if (action === "list") {
        const res = await fetch(`${gatewayUrl}/api/pairing/list`);
        if (!res.ok) throw new Error(`Gateway returned ${res.status}`);
        const codes = (await res.json()) as Array<{ code: string; channelId: string; senderId: string }>;
        if (codes.length === 0) {
        ...
        for (const c of codes) {
          console.log(`  ${c.code} — Channel: ${c.channelId}, Sender: ${c.senderId}`);
        }
        return;
      }
      ...
      if (action === "approve" || action === "deny") {
        if (!code) {
          console.error("❌ You must provide a pairing code to approve/deny.");
          process.exit(1);
        }
        if (action === "approve") {
          // For approve, we fetch the list to find the channel and sender,
          // then we'd ideally validate it. The gateway /api/pairing/validate
          // validates the code and returns the sender info.
          const res = await fetch(`${gatewayUrl}/api/pairing/validate`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ code }),
          });
          if (!res.ok) throw new Error(`Gateway returned ${res.status}`);
          const result = await res.json() as { valid: boolean; channelId?: string; senderId?: string };
          
          if (!result.valid || !result.channelId || !result.senderId) {
            console.error(`❌ Invalid or expired pairing code: ${code}`);
            process.exit(1);
          }

          // Add to allowlist in config
          const chConfig = config.channels[result.channelId];
          if (chConfig) {
            chConfig.allowlist = chConfig.allowlist || [];
            if (!chConfig.allowlist.includes(result.senderId)) {
              chConfig.allowlist.push(result.senderId);
            }
            saveConfig(config);
            console.log(`✅ Approved! Added ${result.senderId} to ${result.channelId} allowlist.`);
          }
        } else {
          // Deny — just validate it and don't add to config (consumes the code)
          const res = await fetch(`${gatewayUrl}/api/pairing/validate`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ code }),
          ...
          if (!res.ok) throw new Error(`Gateway returned ${res.status}`);
          console.log(`❌ Denied pairing code: ${code}`);
        }
        return;
      }

      console.error(`Unknown action: ${action}. Use: list, approve, deny`);
      process.exit(1);
    } catch (err) {
      console.error(`Failed to execute pairing command: ${err instanceof Error ? err.message : String(err)}`);
      console.error("Is the mxclaw gateway running?");
      process.exit(1);
    }
  });
