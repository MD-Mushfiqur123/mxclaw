...
import { loadConfig, saveConfig, getConfigPath, getConfigDir, getWorkspacePath, loadBootstrap, saveBootstrap, getBootstrapSummary, getBootstrapEnvPath, getBootstrapJsonPath, PROVIDER_ENV_KEYS, generateApiToken } from "@mxclaw/core";
import type { MxClawConfig, ProviderRef, ChannelConfig, AgentConfig } from "@mxclaw/core";
...
function generateLaunchdPlist(mxclawBin: string, configPath: string, logPath: string): string {
...
function generateSystemdService(mxclawBin: string, configPath: string): string {
  return `[Unit]
...
function generateWindowsScript(mxclawBin: string, configPath: string, logPath: string): string {
...
function getMxClawBinPath(): string {
...
function printTable(rows: Array<{ left: string; right: string }>): void {
...
function printSummaryTable(summary: Array<{ label: string; value: string; ok: boolean }>): void {
...
export async function runOnboard(): Promise<void> {
  ...
  if (!isNew) {
    ...
    config = {
      version: 1,
      gateway: { host: "127.0.0.1", port: 18700, apiToken, webhookPath: "/gateway/webhook", corsOrigins: ["http://localhost:5173"], wsHeartbeatIntervalMs: 30000 },
      agents: {},
      channels: {},
      bindings: [],
      devices: [],
      voice: { defaultProvider: "system-tts" as const, openaiRealtime: {}, geminiLive: {}, elevenlabs: { voiceId: "21m00Tcm4TlvDq8ikWAM" }, systemTts: {} },
      logging: { level: "info" as const, subsystems: {}, otel: { enabled: false } },
      storage: { type: "jsonl" as const, workspacePath: "~/.mxclaw/workspace", lanceDbPath: "~/.mxclaw/lancedb", sqlitePath: "~/.mxclaw/mxclaw.db" },
      plugins: [],
      sandbox: { enabled: false, type: "docker" as const },
      ownerId: undefined,
    } as unknown as MxClawConfig;
  ...
  const bootstrapEntries: Record<string, string> = loadBootstrap();
  ...
  if (config.gateway?.apiToken) {
    bootstrapEntries["MXCLAW_API_TOKEN"] = config.gateway.apiToken;
  }
  ...
  if (config.gateway?.apiToken) {
    console.log(color(`\n  🔑 ${BOLD}Gateway API Token:${RESET} ${DIM}${config.gateway.apiToken}${RESET}`, CYAN));
    console.log(color(`     ${DIM}Use this token to authenticate the Control UI and API clients.${RESET}`, DIM));
    console.log(color(`     ${DIM}Stored in: ${getBootstrapEnvPath()} (MXCLAW_API_TOKEN)${RESET}`, DIM));
  }
  ...
  console.log(color(`  ${"3.".padEnd(3)} Open ${BOLD}http://localhost:5173${RESET} for the control UI (enter API token on first use)`, DIM));
