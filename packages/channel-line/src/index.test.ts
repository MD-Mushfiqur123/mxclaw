...
describe("line channel", () => {
  ...
  describe("manifest", () => {
    ...
    it("should list sendMessage and receiveMessage capabilities", () => {
      ...
      expect(caps).toContain("flexMessages");
      expect(caps).toContain("stickers");
    });
  });
  describe("setupChannel", () => {
    it("should throw when no credentials provided", async () => {
      const config: ChannelConfig = { id: "line-1", type: "line", enabled: true, credentials: {}, options: {}, allowlist: [], mentionGating: true, pairingEnabled: true };
    ...
    it("should accept credentials from config", async () => {
      ...
      const config: ChannelConfig = { id: "line-1", type: "line", enabled: true, credentials: { token: "test" }, options: {}, allowlist: [], mentionGating: true, pairingEnabled: true };
