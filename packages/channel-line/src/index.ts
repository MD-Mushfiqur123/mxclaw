...
function getState(id: string): LineState {
  let s = states.get(id);
  if (!s) { s = { channelAccessToken: "", channelSecret: "", connected: false, messageCount: 0, queue: [] }; states.set(id, s); }
  return s;
}
...
const plugin: ChannelPlugin = {
  ...
  setupChannel: async (config) => {
    const channelAccessToken = (config.credentials?.channelAccessToken as string) ?? process.env.LINE_CHANNEL_ACCESS_TOKEN;
    const channelSecret = (config.credentials?.channelSecret as string) ?? process.env.LINE_CHANNEL_SECRET;
    if (!channelAccessToken || !channelSecret) throw new Error("LINE channelAccessToken and channelSecret required");
