...
interface TeamsState {
  botId: string; botPassword: string; accessToken: string; tokenExpiry: number;
  connected: boolean; messageCount: number; queue: ReplyEnvelope[];
  onMessage?: (env: MessageEnvelope) => Promise<void>;
}
const states = new Map<string, TeamsState>();

function getState(id: string): TeamsState {
  let s = states.get(id);
  if (!s) { s = { botId: "", botPassword: "", accessToken: "", tokenExpiry: 0, connected: false, messageCount: 0, queue: [] }; states.set(id, s); }
  return s;
}

async function getTeamsToken(state: TeamsState): Promise<string> {
...
const plugin: ChannelPlugin = {
  ...
  setupChannel: async (config) => {
    const botId = (config.credentials?.botId as string) ?? process.env.TEAMS_BOT_ID;
    const botPassword = (config.credentials?.botPassword as string) ?? process.env.TEAMS_BOT_PASSWORD;
    if (!botId || !botPassword) throw new Error("Teams botId and botPassword required");
  ...
  sendMessage: async (id, reply) => {
    ...
    try {
      ...
      const serviceUrl = (reply.metadata?.serviceUrl as string) ?? "https://smba.trafficmanager.net/amer";
