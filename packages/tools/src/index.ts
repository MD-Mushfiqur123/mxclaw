import { z } from "zod";
import type { ToolDefinition, ToolContext, ToolResult, ApprovalRequest } from "@mxclaw/core";
...
export const browserTool: ToolDefinition = {
  ...
  execute: async (args, _context): Promise<ToolResult> => {
    ...
    try {
      ...
      const debugInfo = (await response.json()) as { webSocketDebuggerUrl: string };
      ...
      switch (action) {
        case "navigate": {
          if (!url) return { success: false, output: "", error: "URL required for navigate" };
        ...
        case "evaluate": {
          if (!script) return { success: false, output: "", error: "Script required for evaluate" };
        ...
        case "click": {
          if (!selector) return { success: false, output: "", error: "Selector required for click" };
          ...
          if (!nodeId) return { success: false, output: "", error: `Element not found: ${selector}` };
          const boxModel = await sendCommand("DOM.getBoxModel", { nodeId });
          const quad = (boxModel as { model: { content: number[] } }).model.content;
        ...
        case "getText": {
          if (!selector) return { success: false, output: "", error: "Selector required for getText" };
          const evalResult = await sendCommand("Runtime.evaluate", {
  ...
  execute: async (args, context): Promise<ToolResult> => {
    ...
    switch (action) {
      case "draw": {
        ...
        return {
          success: true,
          output: `Canvas updated with ${(json?.elements as Array<unknown>)?.length ?? 0} elements`,
      ...
      case "update": {
        ...
        const existing = JSON.parse(fs.readFileSync(canvasFile, "utf-8")) as Record<string, unknown>;
...
let cronPersistence: CronPersistence | null = null;

function getCronStore(workspacePath: string): CronPersistence {
  if (!cronPersistence) {
...
let _sessionSpawnFn: SessionSpawnFn | null = null;

/**
 * Register the session spawn handler. Called once by the gateway during startup.
 */
export function registerSessionSpawnHandler(fn: SessionSpawnFn): void {
  _sessionSpawnFn = fn;
}

export const sessionSpawnTool: ToolDefinition = {
  name: "session_spawn",
  description:
    "Spawn a new isolated agent session to handle a subtask. " +
  ...
  execute: async (args, context): Promise<ToolResult> => {
    ...
    try {
      const result = await _sessionSpawnFn(
        context.sessionKey,
        agentId,
        message,
        spawnContext,
...
let _memoryAdapter: MemoryAdapter | null = null;
  ...
  execute: async (args): Promise<ToolResult> => {
    ...
    try {
      switch (action) {
        ...
        case "recall": {
          if (!id) return { success: false, output: "", error: "id required for recall" };
          const entry = await _memoryAdapter.recall(id);
          if (!entry) return { success: false, output: "", error: `Memory not found: ${id}` };
          return { success: true, output: JSON.stringify(entry, null, 2) };
        }
        case "forget": {
          if (!id) return { success: false, output: "", error: "id required for forget" };
...
function zodToJsonSchema(schema: z.ZodType): Record<string, unknown> {
  ...
  function unwrap(s: z.ZodType): { inner: z.ZodType; isOptional: boolean } {
    let inner = s;
  ...
  function fieldToSchema(field: z.ZodType): { schema: Record<string, unknown>; optional: boolean } {
    ...
    if (inner instanceof z.ZodString) {
      ...
      if (objSchema.required) fieldDef.required = objSchema.required;
    ...
    if (inner.description) fieldDef.description = inner.description;
  ...
  for (const [key, field] of Object.entries(shape)) {
    const { schema: fieldSchema, optional } = fieldToSchema(field as z.ZodType);
    properties[key] = fieldSchema;
    if (!optional) requiredFields.push(key);
  }
...
function isPathAllowed(resolved: string, workspacePath: string): boolean {
...
export class ApprovalManager {
  ...
  private defaultTimeoutMs = 60000; // 1 minute
