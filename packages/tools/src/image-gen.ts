import type { ToolDefinition, ToolContext, ToolResult } from "@mxclaw/core";
import { z } from "zod";
...
interface ImageGenProvider {
  name: string;
  generate: (args: z.infer<typeof ImageGenParamsSchema>, apiKey: string) => Promise<{ url?: string; base64?: string; error?: string }>;
}
...
const providers: Record<string, ImageGenProvider> = {
    ...
    generate: async (args, apiKey) => {
      const resp = await fetch("https://api.stability.ai/v2beta/stable-image/generate/core", {
        ...
        body: (() => {
          ...
          if (args.negativePrompt) fd.append("negative_prompt", args.negativePrompt);
  ...
  replicate: {
    ...
    generate: async (args, apiKey) => {
      ...
      const prediction = (await resp.json()) as { id: string; status: string; output?: string[] };
      // Poll for completion
  ...
  local: {
    ...
    generate: async (args, _apiKey) => {
      ...
      try {
        ...
        if (!resp.ok) return { error: `SD WebUI error [${resp.status}]` };
...
function mapSize(w: number, h: number): string {
