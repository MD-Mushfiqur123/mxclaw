...
function getState(id) {
    let s = states.get(id);
    if (!s) {
        s = { relay: "", privateKey: "", privateKeyBytes: new Uint8Array(), publicKey: "", ws: null, connected: false, messageCount: 0, queue: [], subId: null };
        states.set(id, s);
    }
    return s;
}
function packEvent(pubkey, kind, tags, content) {
...
const plugin = {
    ...
    setupChannel: async (config) => {
        ...
        state.relay = config.credentials?.relay ?? "wss://relay.damus.io";
        state.privateKey = config.credentials?.privateKey ?? "";
        if (!state.privateKey)
            throw new Error("Nostr privateKey (hex) required in credentials");
