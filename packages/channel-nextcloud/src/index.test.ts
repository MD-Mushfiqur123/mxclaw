...
describe("nextcloud channel", () => {
  ...
  describe("manifest", () => {
    ...
    it("should list sendMessage and receiveMessage capabilities", () => {
      ...
      expect(caps).toContain("rooms");
      expect(caps).toContain("webhooks");
    });
  });
  describe("setupChannel", () => {
    it("should throw when no credentials provided", async () => {
      const config: ChannelConfig = { id: "nextcloud-1", type: "nextcloud", enabled: true, credentials: {}, options: {}, allowlist: [], mentionGating: true, pairingEnabled: true };
    ...
    it("should accept credentials from config", async () => {
      ...
      const config: ChannelConfig = { id: "nextcloud-1", type: "nextcloud", enabled: true, credentials: { token: "test" }, options: {}, allowlist: [], mentionGating: true, pairingEnabled: true };
