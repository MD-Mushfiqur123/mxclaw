...
describe("googlechat channel", () => {
  ...
  describe("manifest", () => {
    ...
    it("should list sendMessage and receiveMessage capabilities", () => {
      ...
      expect(caps).toContain("spaces");
    });
  });
  describe("setupChannel", () => {
    it("should throw when no credentials provided", async () => {
      const config: ChannelConfig = { id: "googlechat-1", type: "googlechat", enabled: true, credentials: {}, options: {}, allowlist: [], mentionGating: true, pairingEnabled: true };
    ...
    it("should accept credentials from config", async () => {
      ...
      const config: ChannelConfig = { id: "googlechat-1", type: "googlechat", enabled: true, credentials: { token: "test" }, options: {}, allowlist: [], mentionGating: true, pairingEnabled: true };
