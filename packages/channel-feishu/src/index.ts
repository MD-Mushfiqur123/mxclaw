...
interface FeishuState {
  appId: string; appSecret: string; tenantAccessToken: string; tokenExpiry: number;
  connected: boolean; messageCount: number; queue: ReplyEnvelope[];
  onMessage?: (env: MessageEnvelope) => Promise<void>;
}
const states = new Map<string, FeishuState>();

function getState(id: string): FeishuState {
  let s = states.get(id);
  if (!s) { s = { appId: "", appSecret: "", tenantAccessToken: "", tokenExpiry: 0, connected: false, messageCount: 0, queue: [] }; states.set(id, s); }
  return s;
}

async function getTenantToken(state: FeishuState): Promise<string> {
  if (state.tenantAccessToken && Date.now() < state.tokenExpiry) return state.tenantAccessToken;
...
const plugin: ChannelPlugin = {
  ...
  setupChannel: async (config) => {
    const appId = (config.credentials?.appId as string) ?? process.env.FEISHU_APP_ID;
    const appSecret = (config.credentials?.appSecret as string) ?? process.env.FEISHU_APP_SECRET;
    if (!appId || !appSecret) throw new Error("Feishu appId and appSecret required");
