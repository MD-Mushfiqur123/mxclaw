...
function getState(id) {
    let s = states.get(id);
    if (!s) {
        s = { host: "127.0.0.1", port: 6700, ws: null, connected: false, messageCount: 0, queue: [], selfId: 0, echoCounter: 0 };
        states.set(id, s);
    }
    return s;
}
const plugin = {
    ...
    setupChannel: async (config) => {
        ...
        state.host = config.credentials?.host ?? "127.0.0.1";
        state.port = config.credentials?.port ?? 6700;
    },
