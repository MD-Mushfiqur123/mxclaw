...
const states = new Map<string, ZaloState>();

function getState(id: string): ZaloState {
  let s = states.get(id);
  if (!s) { s = { appId: "", appSecret: "", accessToken: "", connected: false, messageCount: 0, queue: [], pollTimer: null, lastMessageId: "", tokenExpiry: 0 }; states.set(id, s); }
  return s;
}

async function refreshToken(state: ZaloState): Promise<void> {
...
const plugin: ChannelPlugin = {
  ...
  setupChannel: async (config) => {
    ...
    if (!state.appId || !state.appSecret) throw new Error("Zalo appId and appSecret required");
  },
    ...
    const poll = async () => {
      ...
      try {
        ...
        const data = (await resp.json()) as {
          error?: number; data?: Array<{ message_id: string; text: string; sender: { id: string }; timestamp: number }>;
        };
        if (data.error) return;
        const messages = data.data ?? [];
        for (const msg of messages) {
          if (msg.message_id === state.lastMessageId || msg.sender.id === state.appId) continue;
