...
interface SignalState {
  restApiUrl: string; connected: boolean; messageCount: number; queue: ReplyEnvelope[];
  pollTimer: ReturnType<typeof setInterval> | null; lastTimestamp: number;
  onMessage?: (env: MessageEnvelope) => Promise<void>;
}
const states = new Map<string, SignalState>();

function getState(id: string): SignalState {
  let s = states.get(id);
  if (!s) { s = { restApiUrl: "", connected: false, messageCount: 0, queue: [], pollTimer: null, lastTimestamp: 0 }; states.set(id, s); }
  return s;
}
...
const plugin: ChannelPlugin = {
  ...
  setupChannel: async (config) => {
    const restApiUrl = (config.credentials?.restApiUrl as string) ?? process.env.SIGNAL_REST_API_URL ?? "http://localhost:8080";
  ...
  startChannel: async (config, onMessage) => {
    ...
    const poll = async () => {
      try {
        const resp = await fetch(`${state.restApiUrl}/v1/receive/${config.options?.account as string ?? "+1234567890"}?timeout=30`);
        const data = (await resp.json()) as { envelope?: { source: string; sourceName: string; dataMessage?: { message: string; timestamp: number; groupInfo?: { groupId: string } } } };
