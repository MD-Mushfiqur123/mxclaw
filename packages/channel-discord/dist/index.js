...
const plugin = {
    ...
    setupChannel: async (config) => {
        const token = config.credentials?.token ?? process.env.DISCORD_BOT_TOKEN;
        if (!token)
            throw new Error("Discord bot token required (credentials.token or DISCORD_BOT_TOKEN)");
    ...
    sendMessage: async (channelId, reply) => {
        ...
        if (!textContent)
            return;
        try {
            const response = await fetch(`https://discord.com/api/v10/channels/${reply.conversationId}/messages`, {
                method: "POST",
                headers: {
    ...
    handleCommand: async (channelId, command, args) => {
        const state = states.get(channelId);
        if (!state)
            return;
        // Handle Discord-specific commands
    },
    handleApproval: async (channelId, approval) => {
        const state = states.get(channelId);
        if (!state)
            return;
...
async function connectDiscord(channelId, state) {
    // Get gateway URL
...
function handleDiscordMessage(channelId, state, data) {
    ...
    const envelope = {
        ...
        threadId: data.message_reference?.message_id,
        content: [
            ...(data.content
                ? [{ type: "text", text: data.content }]
                : []),
            ...(data.attachments ?? []).map((att) => ({
                type: att.content_type?.toString().startsWith("image") ? "image" : "file",
                url: att.url,
                name: att.filename,
            })),
        ],
