/**
 * Sandbox environment variable sanitizer.
 *
 * Adapted from OpenClaw (MIT License) — openclaw/openclaw
 * src/agents/sandbox/sanitize-env-vars.ts
 *
 * Prevents API keys, tokens, and secrets from leaking into sandbox containers.
 */
...
const BLOCKED_ENV_VAR_PATTERNS: ReadonlyArray<RegExp> = [
  ...
  /^MXCLAW_GATEWAY_(TOKEN|PASSWORD)$/i,
...
/**
 * Sanitize host environment variables before passing to sandbox.
 * Blocks secrets; optionally enforces a strict allowlist.
 */
...
/**
 * Sanitize explicit sandbox env vars from config (cfg.sandbox.docker.env).
 * Less strict — allows any key the user explicitly sets, but still blocks
 * null bytes and validates values.
 */
