...
const VERIFIERS: Record<WebhookPlatform, (body: string, headers: Record<string, string>, secret: string) => boolean> = {
  ...
  googlechat: (_body, headers, projectNumber) => {
    ...
    try {
      ...
      if (projectNumber && payload.aud !== projectNumber) return false;
...
export function getWebhookVerifier(platform: WebhookPlatform) {
  return {
    platform,
    verify: (body: string, headers: Record<string, string>, secret: string) =>
      verifyWebhook(platform, body, headers, secret),
  };
}