import { validateDeviceToken, rotateDeviceToken, } from "@mxclaw/security";
 ...
 */
export function handleWebSocketConnection(deps, ws) {
    let deviceId = null;
        ...
        try {
            ...
            switch (msg.type) {
                ...
                case "presence:update":
                    broadcastWs(deps.wsClients, { type: "presence:update", deviceId: deviceId, status: msg.status }, clientId);
                    break;
                case "ping":
                    sendWs(ws, { type: "pong" });
                    break;
...
// ── WS Helpers ───────────────────────────────────────────────────────
export function sendWs(ws, msg) {
    if (ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify(msg));
    }
}
export function broadcastWs(clients, msg, excludeClientId) {
